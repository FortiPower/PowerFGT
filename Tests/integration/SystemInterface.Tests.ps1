...
Describe "Get System Interface" {
    ...
    BeforeAll {
        Add-FGTSystemInterface -name $pester_int1 -role lan -mode static -vdom_interface root -interface $pester_port1 -vlan_id $pester_vlanid1
    }
...
Describe "Add System Interface" {
    ...
    It "Add System Interface with only mandatory parameters" {
        Add-FGTSystemInterface -name $pester_int1 -role lan -mode static -vdom_interface root -interface $pester_port1 -vlan_id 10
        $interface = Get-FGTSystemInterface -name $pester_int1
        $interface.name | Should -Be $pester_int1
    ...
    It "Add System Interface (All parameters...)" {
        Add-FGTSystemInterface -name $pester_int1 -alias Alias_$pester_int1 -role lan -vlan_id $pester_vlanid1 -interface $pester_port1 -admin_access https, ping, ssh -status up -device_identification $true -mode static -ip 192.0.2.1 -netmask 255.255.255.0 -vdom_interface root
        $interface = Get-FGTSystemInterface -name $pester_int1
        $interface.name | Should -Be $pester_int1
        $interface.type | Should -Be "vlan"
