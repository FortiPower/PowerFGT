...
Describe "Connect to a FortiGate (using multi connection)" {
    ...
    Context "Use Multi connection for call some (Get) cmdlet (Vlan, System...)" {
        ...
        It "Use Multi connection for call Get Monitor System HA Peer (< 6.2.0)" -skip:($fgt_version -ge "6.2.0") {
        ...
        It "Use Multi connection for call Get Monitor System HA Checksum (< 6.2.0)" -skip:($fgt_version -ge "6.2.0") {
